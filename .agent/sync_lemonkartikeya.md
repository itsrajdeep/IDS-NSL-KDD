...
> - This avoids merge conflicts entirely â€” each file has exactly one writer.
>
> ### Protocol
...
> - **Entry format**: See the structured block format below. Keep entries compact â€” this is not a conversation transcript, it's a coordination changelog.
>
> ### Entry Format
> ```
...
**Files Changed**:
- `src/contracts.py` â€” NEW: `BaseIDSModel` ABC defining `name`, `fit()`, `predict()`, `evaluate()` contract
- `src/models/base.py` â€” NEW: `IDSModelMixin` with `evaluate()` fully implemented (latency Âµs, FPR, macro/binary auto-detect)
- `src/models/__init__.py` â€” NEW: Dynamic `MODEL_REGISTRY` with `try/except` imports from both `tree_models.py` and `boosting_models.py`; auto-discovers all `BaseIDSModel` subclasses
- `requirements.txt` â€” Added `lightgbm` and `catboost` (were missing)
- `boosting_dev/feature_selection_evolopy.ipynb` â€” Renamed from `boosting_models_scratchpad.ipynb` (name was misleading; notebook actually runs EvoloPy feature selection and produces `feature_subsets.pkl`)
**Decisions Made**:
...
- Registry uses `try/except ImportError` so it works even before `tree_models.py` is pushed â€” no blocking dependency
- `IDSModelMixin` is the class to inherit from (not `BaseIDSModel` directly); it provides `evaluate()` for free
**Depends On**: `data/processed/feature_subsets.pkl`, `level1_binary.npz`, `level2_multiclass.npz` â€” already generated by `boosting_dev/feature_selection_evolopy.ipynb`
**Blocked**: None
